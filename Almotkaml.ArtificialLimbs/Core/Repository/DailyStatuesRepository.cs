...
{
    public class DailyStatuesRepository : Repository<DailyStatues>, IDailyStatuesRepository
    {

        public DailyStatuesRepository(ArtificialLimbsDbContext context)
            : base(context)
        {
        ...
        public int? GetRefNO()
        {
            var MaxNO =  _context.DailyStatues.Max(d => d.ReferenceNo);
            if (MaxNO == null) MaxNO = 0;
            var max = MaxNO + 1;
            return max;
        }

        public IEnumerable<DailyStatues> Search(DailyStatuesModel model, string State)
        {
            var Grid = _context.DailyStatues.Include(d => d.device)
                                           .Include(d => d.patient)
        ...
        public override IEnumerable<DailyStatues> GetAll()
        {
            return _context.DailyStatues.Include(d => d.patient)
                                        .Include(d => d.device)
                                        .Include(d => d.Technical)
                                        .Include(d => d.TechnicalAssistant).ToList();
